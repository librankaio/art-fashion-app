            ...
            <div id="accordion">
                <div class="accordion">
                    <div class="accordion-header" role="button" data-toggle="collapse" data-target="#panel-body-1"
                        aria-expanded="true" style="background-color: #fc544b;">
                        <h4>(Click For Details) Error Item : </h4>
                    </div>
                    <div class="accordion-body collapse show" id="panel-body-1" data-parent="#accordion">
                        @if (session()->has('items_error'))
                            @foreach (session('items_error') as $item)
                                <p class="mb-0">- Item Code: {{ $item }}</p>
                            @endforeach
                        @endif
                    </div>
                </div>
...
@if ($message = Session::get('error_data'))
    <div class="alert alert-danger alert-block">
        <button type="button" class="close" data-dismiss="alert" id="btnclose">×</button>
        <strong>FAILED : {{ $message }}</strong> <br>
        <strong>Counter Pilihan : {{ Session::get('counter_selected') }}</strong>
