<html>
<head><meta http-equiv="Content-Type" content="text/html; charset=utf-8">

	<title>Print - Bon Penjualan</title>
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Open+Sans&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css"
        integrity="sha384-ggOyR0iXCbMQv3Xipma34MD+dH/1fQ784/j6cY/iJTQUOhcWr7x9JvoRxT2MZw1T" crossorigin="anonymous">
</head>
<body class="idr" onload="window.print()">

    <section class="upright" style="width: 80mm;">
        <div style="margin-left: 0%; margin-right: 0%;">
            {{-- <div class="container pt-5">
                <div class="row">
                    <div class="col-6">
                        <h5>Art Fashion Jewelry <br>Telp. (62 - 21) 645 0910/11<br>Fax. (62 - 21) 684288</h5>
                    </div>
                    <div class="col-6 text-right">
                        <h5>Jakarta<br>{{ date("Y-m-d") }}<br>Pameran Paramita Depok<br>Jawa Barat</h5>
                    </div>
                </div>
            </div> --}}
            <div class="container pt-3">
                <center>
                    <img src="{{ asset('../assets/img/cherry.jpg') }}" alt="logo" width="300">
                </center>
                <center>
                    <h1>CHERRY BEKASI</h1>
                </center>
                <center>
                    <h5>Metropolitan Mall Bekasi Lt.3 Jl.KH.Noer Ali</h5>
                    <h5>Bekasi Selatan</h5>
                </center>
                <center>
                    <h5>INV/CH001/2308202/000252</h5>
                </center>
                <hr style="border-top: dotted 0.3px;" />
                <div class="row">
                    <div class="col-12">
                        <h5>Pelanggan : CHERRY BEKASI</h5>
                        <h5>TRANSAKSI : 28 AGT 2023, 10.18</h5>
                        <h5>KARYAWAN : DARMIA SEMBIRING</h5>
                    </div>
                </div>
                <hr style="border-top: dotted 0.3px;" />
                <div class="row">
                    <div class="col-12">
                        <h5>YRSSQM00001355 Ikat Rambut</h5>
                        <div class="row">
                            <div class="col-6">
                                <h5>1 Pcs x Rp.5,000.00</h5>
                            </div>
                            <div class="col-6 d-flex justify-content-end align-items-end">
                                <h5>Rp.15,000.00 </h5>
                            </div>
                        </div>
                    </div>
                    <div class="col-12">
                        <h5>YRSSQM00001355 Ikat Rambut</h5>
                        <div class="row">
                            <div class="col-6">
                                <h5>1 Pcs x Rp.5,000.00</h5>
                            </div>
                            <div class="col-6 d-flex justify-content-end align-items-end">
                                <h5>Rp.15,000.00 </h5>
                            </div>
                        </div>
                    </div>
                </div>
                <hr style="border-top: dotted 0.3px;" />
                <div class="row">
                    <div class="col-6 d-flex justify-content-end align-items-end">
                        <h5>Total Barang : </h5>
                    </div>
                    <div class="col-6 d-flex justify-content-end align-items-end">
                        <h5>5</h5>
                    </div>
                </div>
                <hr style="border-top: dotted 0.3px;" />
                <div class="row">
                    <div class="col-6 d-flex justify-content-end align-items-end">
                        <h5>Subtotal : </h5>
                    </div>
                    <div class="col-6 d-flex justify-content-end align-items-end">
                        <h5>15,000.00</h5>
                    </div>
                </div>
                <hr style="border-top: dotted 0.3px;" />
                <div class="row">
                    <div class="col-6 d-flex justify-content-end align-items-end">
                    </div>
                    <div class="col-6 d-flex justify-content-end align-items-end">
                        <h3>TOTAL : RP.15,000.00</h3>
                    </div>
                </div>
                <hr style="border-top: dotted 0.3px;" />
                <div class="row">
                    <div class="col-6 d-flex justify-content-end align-items-end">
                        <h5>Tunai : </h5>
                    </div>
                    <div class="col-6 d-flex justify-content-end align-items-end">
                        <h5>15,000.00</h5>
                    </div>
                </div>
                <hr style="border-top: dotted 0.3px;" />
                <div class="row">
                    <div class="col-6 d-flex justify-content-end align-items-end">
                        <h5>Kembali : </h5>
                    </div>
                    <div class="col-6 d-flex justify-content-end align-items-end">
                        <h5>0</h5>
                    </div>
                </div>
                <hr style="border-top: dotted 0.3px;" />
                <center>
                    <h4>** Terima Kasih **</h4>
                </center>
                <hr style="border-top: dotted 0.3px;" />
                <h5>Instagram : atfashion.official</h5>
                <h5>Facebook : officialbrandartfashion</h5>
            </div>
    </section>

</body>
</html>

<style type="text/css" media="print">
    body {
        /* font-family: 'Roboto';font-size: 16px; */
        font-family: 'Open Sans', sans-serif;        
    }
    h1,h2,h3,h4,h5,h6 {
        font-family: 'Open Sans', sans-serif;
    }
    /* @page { size: landscape; margin: 0px auto; } */
    @page {
    page-orientation: upright; 
    margin: 0px auto;
    width: 10mm;
    }

@page upright {
  size: portrait;
  page-orientation: upright;
  width: auto;
}

@media print {
  .upright {
    page: upright;
  }
}

.border-dotted{
    border-style: dotted;
}
</style>


