    ...
    public function list(){
        ...
        if(session('privilage') == null){
            if (isset(request()->search)) {
                $tpenjualanhs = Tpenjualan_h::select('id','no','tgl','counter','note','payment_mthd','noreff','grdtotal','hrgsblmdisc','diskon')->where('counter','=',session('counter'))->where('no','LIKE','%'.request()->search.'%')->orderBy('created_at', 'asc')->paginate(50);
                $tpenjualands = Tpenjualan_d::select('id','idh','no_penjualan','code','name','qty','satuan','hrgjual','diskon','subtotal','note',)->get();
            }else{
                $tpenjualanhs = Tpenjualan_h::select('id','no','tgl','counter','note','payment_mthd','noreff','grdtotal','hrgsblmdisc','diskon')->where('counter','=',session('counter'))->orderBy('created_at', 'asc')->paginate(50);
                $tpenjualands = Tpenjualan_d::select('id','idh','no_penjualan','code','name','qty','satuan','hrgjual','diskon','subtotal','note',)->get();
            }
        }else if (session('privilage') == 'ADM'){
            if(Request()->counter_filter != null){
                if (isset(request()->search)) {
                    $tpenjualanhs = Tpenjualan_h::select('id','no','tgl','counter','note','payment_mthd','noreff','grdtotal','hrgsblmdisc','diskon')->where('counter','=',session('bonjual_counter'))->where('no','LIKE','%'.request()->search.'%')->orderBy('created_at', 'asc')->paginate(50);
                    $tpenjualands = Tpenjualan_d::select('id','idh','no_penjualan','code','name','qty','satuan','hrgjual','diskon','subtotal','note',)->get();
                }else{
                    ...
                    $tpenjualanhs = Tpenjualan_h::select('id','no','tgl','counter','note','payment_mthd','noreff','grdtotal','hrgsblmdisc','diskon')->where('counter','=',session('bonjual_counter'))->orderBy('created_at', 'asc')->paginate(50);
                    $tpenjualands = Tpenjualan_d::select('id','idh','no_penjualan','code','name','qty','satuan','hrgjual','diskon','subtotal','note',)->get();
                }
            }else{
                if (isset(request()->search)) {
